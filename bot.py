...
RENDER_URL = os.environ.get("RENDER_EXTERNAL_URL", "https://bot-telegram-api-bot-4.onrender.com")
...
def send_main_menu(chat_id):
    ...
    markup.add(types.InlineKeyboardButton("🔑 Key и ман", callback_data="check_mykey"))
...
def send_admin_menu(chat_id):
    ...
    markup.add(types.InlineKeyboardButton("👁 Вақти Key ҳо", callback_data="admin_viewkeys"))
...
def callback_handler(call):
    ...
    if call.data == "check_sub":
    ...
    elif call.data == "check_mykey":
        not_sub = check_subscription(user_id)
        if not_sub:
            bot.answer_callback_query(call.id, "❌ Аввал обуна шав!", show_alert=True)
            return
        user_uid_state[user_id] = "waiting_mykey"
        markup = types.InlineKeyboardMarkup()
        markup.add(types.InlineKeyboardButton("🔙 Бозгашт", callback_data="back_menu"))
        bot.edit_message_text(
            "🔑 *Санҷиши Key*\n\nKey иро фирист:",
        ...
        for k, username, expires_at, used, active in keys:
            ...
            text += (
                f"{status} @{username}\n"
                f"🔑 `{k}`\n"
                f"📅 Тамом: `{expires.strftime('%Y-%m-%d')}`\n"
                f"⏱ Мемонад: `{max(0, remaining)}` рӯз\n"
                f"📊 Request: `{used}`/`{'∞' if active == -1 else active}`\n\n"
            )
        bot.edit_message_text(text, chat_id, msg_id, parse_mode="Markdown", reply_markup=markup)
...
def handle_messages(msg):
    ...
    if user_uid_state.get(user_id) == "admin_waiting_key_info" and is_admin(user_id):
    ...
    elif user_uid_state.get(user_id) == "waiting_mykey":
        del user_uid_state[user_id]
        key = text.strip()
        info = get_key_info(key)
        markup = types.InlineKeyboardMarkup()
        markup.add(types.InlineKeyboardButton("🔙 Бозгашт", callback_data="back_menu"))
        if not info:
            bot.send_message(chat_id, "❌ Key вуҷуд надорад!", reply_markup=markup)
            return
        _, _, user_id2, username, created_at, expires_at, max_req, used_req, is_active = info
        expires = datetime.fromisoformat(expires_at)
        remaining = (expires - datetime.now()).days
        status = "✅ Фаъол" if is_active and remaining >= 0 else "❌ Тамом"
        req_info = f"{used_req}/{'∞' if max_req == -1 else max_req}"
        bot.send_message(chat_id,
            f"🔑 *Маълумоти Key:*\n\n"
            f"📌 Ҳолат: {status}\n"
            f"👤 Соҳиб: @{username}\n"
            f"🔑 Key:\n`{key}`\n"
            f"📅 Сохта шуд: `{created_at[:10]}`\n"
            f"⏱ Тамом: `{expires.strftime('%Y-%m-%d')}`\n"
            f"🕐 Мемонад: `{max(0, remaining)}` рӯз\n"
            f"📊 Request: `{req_info}`\n\n"
            f"🌐 *Истифода:*\n"
